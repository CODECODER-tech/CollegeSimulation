...
void draw_map()
{
    GLuint Texture = loadBMP_custom("D:/CGPROJECT/CollegeSimulation/field.bmp");
...
void draw_idol()
{
    GLuint Texture = loadBMP_custom("D:/CGPROJECT/CollegeSimulation/Vishnu.bmp");
    glEnable(GL_TEXTURE_2D);
    GLuint textureID;
    glGenTextures(1, &textureID);
    glBindTexture(GL_TEXTURE_2D, textureID);
...
public:
    ...
    void disp_build(const char text[15], const char side = '/0')
    {
        float door[3];
...
building canteen(20, 40, -40);
building mech(20, 40, -40);
building mba(20, 40, -40);
building admin(30, 20, -20);
building ec(20, 40, -40);
building cs(20, 40, -40);


void disp_cse()		//cse block interiors
{
    int k;
    glPushMatrix();
    glTranslatef(50, 0.1, 0);
    glColor3f(0.5, 0.5, 0.5);
    glBegin(GL_QUADS);
    glVertex3f(0, 0, 0);
    glVertex3f(20, 0, 0);
...
void disp_ec()		//ec block interiors
{
    int k;
    glPushMatrix();
    glTranslatef(50, 0.1, 50);
    glColor3f(0.5, 0.5, 0.5);
    glBegin(GL_QUADS);
    glVertex3f(0, 0, 0);
    glVertex3f(20, 0, 0);
...
void disp_canteen()		//CANTEEN block interiors
{
    int k;
    glPushMatrix();
    glTranslatef(-70, 0.1, 85);
    glColor3f(0.5, 0.5, 0.5);
    glBegin(GL_QUADS);
    glVertex3f(0, 0, 0);
    glVertex3f(20, 0, 0);
...
void draw_arch(const char text[5])		//draw the arch
{
...
void display()
{
    ...
    mech.disp_build("  SPORTS", 'r');//SPORTS DEPARTMENT
    glPopMatrix();
    mech_court();
    glPushMatrix();
    glTranslatef(-70, 0, -30);
    mba.disp_build("CSE BLOCK", 'r');//MECHANICAL DEPARTMENT
    glPopMatrix();
    disp_mba();
    glPushMatrix();
    ...
    ec.disp_build("EC BLOCK",'l');
    glPopMatrix();
    disp_ec();
    glPushMatrix();
    glTranslatef(50, 0, 0);
    cs.disp_build("MECH BLOCK", 'l');//MECH DEPARTMENT
    glPopMatrix();
    disp_cse();
    glPushMatrix();
...
void frontscreen(void)
{
    ...
    drawstring(532 - 50, 40, 0.0, "SJM VIDYAPEETHA(R)");
    drawstring(362 - 50, 64, 0.0, "SJM INSTITUTE OF TECHNOLOGY, CHITRADURGA");
    glColor3f(1, 0.5, 0);
    drawstring(312 - 50, 88, 0.0, "DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(552 - 50, 122, 0.0, "MINI PROJECT ON");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(396 - 50, 150, 0.0, "COMPUTER GRAPHICS AND VISUALIZATION");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(479 - 50, 182, 0.0, "TOPIC: COLLEGE SIMULATION");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(529 - 50, 216, 0.0, "PROJECT ASSOCIATES");
    glColor3f(0.5, 0.2, 0.2);
    drawstring(489 - 50, 246, 0.0, "KUNAL KUMAR (4SM17CS036)");
    glColor3f(0.5, 0.2, 0.2);
    drawstring(495 - 50, 272, 0.0, "VENKATESH N (4SM17CS081)");
    glColor3f(0.5, 0.2, 0.2);
    drawstring(75 - 50, 330, 0.0, "UNDER THE GUIDANCE OF");
    glColor3f(0.5, 0.2, 0.2);
    drawstring(75 - 50, 360, 0.0, "Mrs.M K SHRUTHI");
    glColor3f(0.5, 0.2, 0.2);
    drawstring(75 - 50, 394, 0.0, "Asso.Prof Dept of CSE");
    glColor3f(0.5, 0.2, 0.2);
    drawstring(75 - 50, 428, 0.0, "SJMIT,CHITRADURGA");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(910 - 50, 330, 0.0, "UNDER THE GUIDANCE OF");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(910 - 50, 360, 0.0, "Mr. PRAVEEN S");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(910 - 50, 394, 0.0, "Asst. Prof Dept of CSE");
    glColor3f(0.1f, 0.1f, 0.0f);
    drawstring(910 - 50, 428, 0.0, "SJMIT,CHITRADURGA");
    glColor3f(1, 0.1, 1);
    drawstring(532 - 50, 580, 0.0, "PRESS ENTER TO START");
    glMatrixMode(GL_PROJECTION);
    glPopMatrix();
    glMatrixMode(GL_MODELVIEW);
    glPopMatrix();
...
void pressKey(unsigned char key, int xx, int yy)
{
    ...
    if (key == 13)
        switch (key) {
        case 13: if (flag == 0) flag = 1; mydisplay(); break;
        }
    else
    {
        switch (key) {
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("College Simulation");
